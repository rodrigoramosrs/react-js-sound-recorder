  ...
  function PKUI_FX(app) {
    ...
    app.listenFor("RequestFXUI_Gain", function () {
      ...
      var x = new PKAudioFXModal(
        {
          id: filter_id,
          title: t("Aplicar ganho para no intervalo selecionado"),

          presets: [
            { name: t("Silêncio"), val: 0 },
            { name: "-50%", val: 0.5 },
            { name: "-25%", val: 0.75 },
            { name: "+25%", val: 1.25 },
            { name: "+50%", val: 1.5 },
          ...
          body:
            '<div class="pk_row" style="border:none"><label>' +
            t("Percentual de ganho") +
            "</label>" +
      ...
      var x = new PKAudioFXModal(
        {
          ...
          presets: [
            { name: "-1/4", val: 0.25 },
            { name: "-1/2", val: 0.5 },
            { name: t("Um pouco mais lento"), val: 0.85 },
            { name: t("Pouco mais rápido") + "%", val: 1.1 },
            { name: "+1/4", val: 1.25 },
            { name: "+1/2", val: 1.5 },
          ],
          custom_pres: custom_presets.Get(filter_id),
    ...
    app.listenFor("RequestFXUI_Silence", function () {
      var x = new PKSimpleModal({
        ...
        body:
          ...
          '<label  for="ifeq">' +
          t("Inserir silencio no inicio") +
          "</label><br/>" +
          '<input type="radio" class="pk_check"  id="vgdja" name="rdslnc" checked value="cursor">' +
          '<label for="vgdja">' +
          t("Inserir silêncio na posição do cursor") +
          ' (<span class="pkcdpk"></span>)</label></div>' +
          '<div class="pk_row"><label>' +
          t("Silêncio em segundos") +
          "</label>" +
        ...
        setup: function (q) {
          ...
          cursor_pos_el.innerHTML =
            PKAudioEditor.engine.wavesurfer.getCurrentTime().toFixed(2) + "s";
        ...
        buttons: [
          {
            ...
            callback: function (q) {
              ...
              var toggle = q.el_body.getElementsByClassName("pk_check")[0]
                .checked;
    ...
    app.listenFor("RequestActionID3", function (flag, new_tags) {
      ...
      var render_tags = function (el, tags) {
        ...
        str +=
          '<div><span class="pk_id3ttl">Artist</span><span>' +
          (tags.artist || "-") +
          "</span></div>";
        str +=
          '<div><span class="pk_id3ttl">Title</span><span>' +
          (tags.title || "-") +
          "</span></div>";
        str +=
          '<div><span class="pk_id3ttl">Album</span><span>' +
          (tags.album || "-") +
          "</span></div>";
        str +=
          '<div><span class="pk_id3ttl">Year</span><span>' +
          (tags.year || "-") +
          "</span></div>";
        str +=
          '<div><span class="pk_id3ttl">Genre</span><span>' +
          (tags.genre || "-") +
          "</span></div>";
        str +=
          ...
          "</span></div>";
        str +=
          '<div><span class="pk_id3ttl">Track</span><span>' +
          (tags.track || "-") +
          "</span></div>";
        str +=
    ...
    app.listenFor("RequestShowFreqAn", function (url, args_arr) {
      ...
      if (!type) {
        var makePopup = function (dat) {
          ...
          if (!wnd) {
            OneUp(
              "Porfavor permita pop-ups for o editor de audio!",
              3600,
              "pk_r"
            );
            return;
