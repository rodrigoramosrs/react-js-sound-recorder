      ...
      for (var i = 0, u = 0; i < wavesurfer.ActiveChannels.length; ++i) {
        ...
        emptySegment
          ...
          .set(
            originalBuffer
              .getChannelData(i)
              .slice(new_offset, new_len + new_offset)
          );
      ...
      if (
        ...
        for (var i = 0; i < originalBuffer.numberOfChannels; ++i) {
          ...
          if (wavesurfer.ActiveChannels[i] === 0) {
            ...
            uber_chan_data.set(
              chan_data.slice(new_offset + new_len),
              new_offset + new_len
            );
          }
        }
    ...
    function InsertSegmentToBuffer(_offset, buffer) {
      ...
      for (var i = 0; i < originalBuffer.numberOfChannels; ++i) {
        ...
        if (_offset < originalBuffer.length + buffer.length) {
          uberChanData.set(
            chan_data.slice(_offset),
            _offset + segment_chan_data.length
          );
        }
      }
      ...
      return [
        ...
        _offset / originalBuffer.sampleRate +
          buffer.length / originalBuffer.sampleRate,
      ];
    }
    ...
    function ReplaceFloatArrays(_offset, arrays) {
      ...
      for (var i = 0; i < originalBuffer.numberOfChannels; i++) {
        ...
        if (_offset < originalBuffer.length + new_len) {
          uberChanData.set(
            chan_data.slice(_offset + new_len),
            _offset + new_len
          );
        }
      }
      ...
      return [
        _offset / originalBuffer.sampleRate,
        _offset / originalBuffer.sampleRate +
          new_len / originalBuffer.sampleRate,
      ];
    }
      ...
      if (this.previewing === 2) {
        ...
        if (this.PreviewFilter) {
          if (this.PreviewFilter.length > 0) {
            ...
            !this.PreviewFilter.buffer &&
              this.PreviewSource.connect(this.PreviewFilter);
            this.PreviewFilter.disconnect();
      ...
      var offline_callback = function (rendered_buffer) {
        ...
        for (var i = 0; i < orig_buffer.numberOfChannels; ++i) {
          ...
          uber_chan_data.set(
            fx_chan_data,
            new_offset,
            fx_chan_data.length - new_offset
          );
        }
...
*/
    ...
    function DownloadFile(
      ...
      worker.postMessage(dataAsInt16ArrayLeft.buffer, [
        dataAsInt16ArrayLeft.buffer,
      ]);
      if (data_right)
        worker.postMessage(dataAsInt16ArrayRight.buffer, [
          dataAsInt16ArrayRight.buffer,
        ]);
      ...
      FadeIn: function (val) {
        return {
          filter: function (audio_ctx, destination, source, duration) {
            ...
            gain.gain.linearRampToValueAtTime(
              1,
              audio_ctx.currentTime + duration / 1
            );
            gain.connect(destination);
            source.connect(gain);
              ...
              if (val[k].length) {
                for (var i = 0; i < val[k].length; ++i) {
                  ...
                  compressor[k].linearRampToValueAtTime(
                    curr.val,
                    audio_ctx.currentTime + curr.time
                  );
                }
              } else {
      ...
      Reverb: function (val) {
        return {
          filter: function (audio_ctx, destination, source, duration) {
            ...
            var impulse = audio_ctx.createBuffer(
              2,
              length,
            ...
            for (i = 0; i < length; i++) {
              ...
              impulseL[i] =
                (Math.random() * 2 - 1) * Math.pow(1 - n / length, val.decay);
              impulseR[i] =
                (Math.random() * 2 - 1) * Math.pow(1 - n / length, val.decay);
            }
