...
//			audio_el.playbackRate = 0.5;
    ...
    app.listenFor("RequestStop", function (val) {
      ...
      if (region)
        wavesurfer.ActiveMarker = region.start / wavesurfer.getDuration();
      ...
      if (!custom) {
        ...
        if (!wavesurfer.isPlaying())
          setTimeout(function () {
            app.fireEvent(
              "RequestSeekTo",
              custom[0] / wavesurfer.getDuration()
            );
          }, 0);
      }
    });
    ...
    app.listenFor("RequestActionRecordStart", function () {
      ...
      var region = wavesurfer.regions.list[0];
      if (region)
        app.rec.setEndingOffset(
          region.end * wavesurfer.backend.buffer.sampleRate
        );
      else
        app.rec.setEndingOffset(
          wavesurfer.getDuration() * wavesurfer.backend.buffer.sampleRate
        );
    });
          ...
          for (var j = 0; j < new_channel.length; ++j) {
            ...
            if (h > sil_curr_start && h < sil_curr_end) {
              if (h < sil_curr_start + jump) {
                ...
                new_channel[j] +=
                  (1 - perc) *
                  channel[j + (sil_offset + (sil_curr_end - sil_curr_start))];
