...
*/
      ...
      if (modified) {
        if (compute_coords) {
          ...
          if (range.type === "peaking")
            range._coords.y =
              (
                (1.0 - (range.gain + max_db_val) / (max_db_val * 2)) *
                ch
              ).toFixed(1) / 1;
    ...
    function _make_ui(q) {
      ...
      var ctx_bars = canvas_bars.getContext("2d", {
        alpha: true,
        antialias: false,
      });
      var ctx_eq = canvas_eq.getContext("2d", {
        alpha: true,
        antialias: false,
      });
    ...
    function _range_render_el(q, range, clss) {
      ...
      el_q.innerHTML =
        '<input type="number" class="pk_val pk_q" min="1" max="50" value="' +
        range.q +
        '">';
    ...
    var x = new PKAudioFXModal(
      {
        ...
        presets: [
          {
            name: "Old Telephone",
            val: "1,highpass,0,5800,5.8,1,lowpass,0,7060,5",
          },
        ],
        ...
        onpreset: function (val) {
          ...
          for (var j = 0; j < els; ++j) {
            ...
            if (curr[3] < 5000) {
              ...
              x =
                (cw / 2 + ((curr[3] - 5000) / 15000) * (cw / 2)).toFixed(1) / 1;
            }

            if (curr[1] === "peaking")
              y =
                (
                  (1.0 - (curr[2] / 1 + max_db_val) / (max_db_val * 2)) *
                  ch
                ).toFixed(1) / 1;
  ...
  var TempoToolsModal = function (app) {
    ...
    var TempoMetro = function (app, modal) {
      ...
      function _make_evs(q) {
        ...
        var metronome_btn = q.body.getElementsByClassName(
          "pk_modal_a_bottom"
        )[0];
          ...
          btn_start.onclick = function () {
            ...
            navigator.mediaDevices
              ...
              .then(function (stream) {
                audio_stream = stream;
                media_stream_source = audio_context.createMediaStreamSource(
                  stream
                );
